                                    ...
                                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
                                        ...
                                        @if ($myTicket->status != 3 && $myTicket->user)
                                            <button class="btn btn--danger close-button btn--sm confirmationBtn" type="button"
                                                data-question="@lang('Êtes-vous sûr de fermer ce ticket?')"
                                            ...
                                            <form method="post" action="{{ route('ticket.reply', $myTicket->id) }}"
                                                ...
                                                <div class="text-end mb-3">
                                                    <a href="javascript:void(0)" class="btn btn--base btn--sm addFile"><i
                                                            class="fa fa-plus"></i> @lang('Ajouter un nouveau')</a>
                                                </div>
                                                <div class="form-group mb-4">
                                                    ...
                                                    <p class="my-2 ticket-attachments-message text-muted">
                                                        @lang('Extensions de fichiers autorisées'): .@lang('jpg'), .@lang('jpeg'),
                                                        .@lang('png'), .@lang('pdf'), .@lang('doc'),
                                                        .@lang('docx')
                                                    </p>
                                                    <label class="form--label">@lang('Pièces jointes')</label>
                                                </div>
                                                <button type="submit" class="btn btn--base btn--sm"> <i
                                                        class="fa fa-reply"></i> @lang('Répondre')</button>
                                            </form>
                                        @endif
                                    </div>
    ...
    <script>
        (function($) {
            ...
            $('.addFile').on('click', function() {
                if (fileAdded >= 4) {
                    notify('error', 'Vous avez ajouté le nombre maximum de fichiers');
                    return false;
                }
                fileAdded++;
