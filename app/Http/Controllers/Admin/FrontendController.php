...
class FrontendController extends Controller
{
    ...
    public function templatesActive(Request $request)
    {
        ...
        $notify[] = ['success', strtoupper($request->name).' modèle activé avec succès'];
        return back()->withNotify($notify);
    }

    public function seoEdit()
    {
        $pageTitle = 'Configuration SEO';
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        foreach ($request->except('_token', 'video') as $input_field => $val) {
            if ($input_field == 'has_image' && $imgJson) {
                foreach ($imgJson as $imgValKey => $imgJsonVal) {
                    ...
                    $validation_message['image_input.'.$imgValKey.'.mimes'] = keyToTitle($imgValKey).' type de fichier non pris en charge';
                }
                continue;
        ...
        if ($type == 'data') {
            ...
            if ($request->hasFile('image_input')) {
                try {
                    ...
                    $notify[] = ['error', 'Impossible de télécharger l\'image'];
                    return back()->withNotify($notify);
                }
            }
        }else{
        ...
        $content->save();
        $notify[] = ['success', 'Le contenu a été mis à jour avec succès'];
        return back()->withNotify($notify);
    }
